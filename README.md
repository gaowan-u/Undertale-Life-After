...
1. 优化了开场动画的退出逻辑，确保按下ESC键能正确跳过动画 （2025-11-16）
...
为了应对在不同平台上的性能差异，本项目采用以下分支策略：

- **`main`**: 此分支将专注于未来的 **移动端 (Android)** 版本开发。目标是将其打包成独立的 APK 文件，并进行针对移动设备的性能优化。

- **`feat/pc-optimization`**: 此分支为 **PC (Windows/Linux/macOS)** 版本。它将包含针对桌面环境的优化，并为希望在电脑上获得最佳体验的玩家提供支持。
